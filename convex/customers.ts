...
// Set customer balance directly (admin function)
export const setCustomerBalance = mutation({
  args: {
    id: v.id("customers"),
    balance: v.number(),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.balance < 0) throw new Error("Balance cannot be negative.");
    
    const oldBalance = customer.balance;
    const difference = oldBalance - args.balance;
    ...
    // If balance was reduced, create a deduction order
    if (difference > 0) {
      await ctx.db.insert("orders", {
        ...
        cashierCode: "SYSTEM",
        cashierName: "Admin",
        clientOrderId: `DEDUCT-${Date.now()}`,
        createdAt: Date.now(),
      });
    }
    
    return { success: true, deductionCreated: difference > 0, amount: difference };
  },
});
