...
export const getAllTimeSales = query({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (startTime >= endTime) {
        throw new Error("Invalid date range: startOfMonth must be before endOfMonth");
      }
      ...
      for (const order of allOrdersRaw) {
        try {
          ...
          salesByCashier[code].revenue += orderTotal;
          salesByCashier[code].orders += 1;

          // Safely handle date creation
          ...
          console.error("Error processing order in getAllTimeSales:", order, orderErr);
          continue;
        }
      }
      ...
      console.error("[getAllTimeSales] Error:", error);
