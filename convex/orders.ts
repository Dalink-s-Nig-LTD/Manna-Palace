...
export const createOrder = mutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      // Primary dedup: check by clientOrderId
      if (args.clientOrderId) {
        ...
        if (existingByClientId) {
          console.log("⚠️ DUPLICATE blocked by clientOrderId:", {
            clientOrderId: args.clientOrderId,
            existingOrderId: existingByClientId._id,
            existingCreatedAt: existingByClientId.createdAt,
            newTotal: args.total,
      ...
      // Fallback dedup: same cashier + same total within a ±10 s window
      // (handles edge cases where clientOrderId was somehow not stored)
      const windowStart = preciseCreatedAt - 10_000;
      const windowEnd   = preciseCreatedAt + 10_000;
      const existingOrder = await ctx.db
        ...
        .filter((q) =>
          q.and(
            ...
            q.lte(q.field("createdAt"), windowEnd),
      ...
      // Ensure every order has a clientOrderId (server-side fallback)
      const finalClientOrderId = args.clientOrderId || `server_${preciseCreatedAt}_${Math.random().toString(36).substr(2, 9)}`;

      // If we generated a server-side clientOrderId, check for dedup with it too
      if (!args.clientOrderId) {
        console.log("⚠️ Order received WITHOUT clientOrderId, generated server-side:", finalClientOrderId);
      }
...
// Check if an order with a given clientOrderId exists (for client-side dedup verification)
export const findByClientOrderId = query({
  ...
  handler: async (ctx, args) => {
    const existing = await ctx.db
      ...
      .withIndex("by_clientOrderId", (q) => q.eq("clientOrderId", args.clientOrderId))
      .first();
    return existing ? { _id: existing._id, exists: true } : { exists: false };
  },
});
