...
export const requestPasswordReset = action({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.runMutation(
      api.passwordReset.generateAndStoreOTP,
      {
        email: args.email,
      },
    );
...
<body style="margin:0;padding:0;background:#fffcf5;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;">
  <div style="max-width:520px;margin:28px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,0.08);border:1px solid #fde68a;">
    <div style="background:#f59e0b;padding:22px 20px;text-align:center;">
      <h1 style="margin:0;color:#fff;font-size:22px;font-weight:800;">Manna Palace</h1>
    ...
    <div style="padding:28px 22px;">
      ...
      <p style="margin:0 0 20px;color:#4b5563;font-size:14px;line-height:1.6;">
        Use the code below to reset your Manna Palace admin password.
      </p>
    ...
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        to: [{ email: user.email }],
        subject: "Password Reset Code - Manna Palace",
        htmlContent,
      }),
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to send reset email: ${response.status} - ${errorText}`,
      );
    }
