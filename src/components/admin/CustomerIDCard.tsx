...
export function CustomerIDCard({ student }: CustomerIDCardProps) {
  ...
  const handlePrint = () => {
    ...
    doc.write(`<html><head><title>ID Card</title><style>
      ...
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      .front-watermark { position:absolute; bottom:-8mm; right:-4mm; font-size:36mm; font-weight:bold; color:rgba(44,24,0,0.06); font-family:Georgia,serif; line-height:1; }
      ...
      .back-watermark { position:absolute; bottom:-6mm; right:-2mm; font-size:28mm; font-weight:bold; color:rgba(180,145,26,0.09); font-family:Georgia,serif; line-height:1; }
      ...
      <div class="page back">
        ...
        <div class="back-right">
          <div class="back-watermark">MP</div>
          <div>
            <div class="back-name">${student.firstName} ${student.lastName}</div>
            <div class="back-sub">Manna Palace Member</div>
          </div>
          <div class="info-row">
          ...
          <div class="back-footer">MANNA PALACE CAFETERIA</div>
        </div>
      </div>
    </body></html>`);
    doc.close();
  ...
  const generatePdfBase64 = async (): Promise<string> => {
    ...
    doc.addImage(frontCanvas.toDataURL("image/png"), "PNG", 0, 0, cardW, cardH);
    doc.addPage([cardW, cardH], "landscape");
    const backCanvas = await html2canvas(backRef.current, {
      scale: 3,
      useCORS: true,
    ...
    doc.addImage(backCanvas.toDataURL("image/png"), "PNG", 0, 0, cardW, cardH);
