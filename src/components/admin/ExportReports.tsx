...
export function ExportReports() {
  ...
  const printReceiptStylePDF = () => {
    ...
    const W = 42; // 80mm paper width (~42 chars)
    const SEP = '-'.repeat(W);
    ...
    lines.push(SEP);
    lines.push('');
    lines.push(center(`Printed: ${format(new Date(), "dd/MM/yyyy HH:mm")}`));
    ...
    </style></head><body><pre style="font-family:'Courier New',Courier,monospace;font-size:9.5px;font-weight:700;margin:0;padding:1mm 2mm;white-space:pre;overflow:hidden;width:100%;max-width:76mm;box-sizing:border-box;">${reportText}</pre></body></html>`);
    printDoc.close();
