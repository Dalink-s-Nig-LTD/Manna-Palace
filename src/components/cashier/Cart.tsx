...
import { BarcodeScanPayment } from "./BarcodeScanPayment";
...
export function Cart() {
  ...
  const [specialOrderPaymentMethod, setSpecialOrderPaymentMethod] = useState<
    "cash" | "card" | "transfer"
  >("cash");
  ...
  return (
    <>
      <Card className="w-full h-full flex flex-col border-border shadow-card bg-white overflow-hidden">
      ...
      <BarcodeScanPayment
        isOpen={showWalletPay}
        onClose={() => setShowWalletPay(false)}
        cartTotal={total}
        onPaymentComplete={(customerId: Id<"customers">) => {
          const order = completeOrder("customer_balance");
          if (order) {
            setLastOrder(order);
            setShowReceipt(true);
          }
