...
export function ReceiptModal({ order, isOpen, onClose }: ReceiptModalProps) {
  ...
  const triggerBrowserPrint = (type?: "food" | "drinks") => {
    ...
    const content = sections.map(s => `<pre style="font-family:'Courier New',Courier,monospace;font-size:9.5px;font-weight:600;margin:0;padding:1mm 2mm;white-space:pre;overflow:hidden;width:100%;box-sizing:border-box;">${s}</pre>`).join('<div style="page-break-before:always"></div>');
