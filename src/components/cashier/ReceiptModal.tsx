...
// Helper to format order number as 'MP-DDMMYY-XXXX'
function formatOrderNumber(order: Order) {
  if (/^MP-\d{6}-[A-Z0-9]{4}$/.test(order.id)) return order.id;
  const date =
  ...
  let suffix = "";
  const seed = order.id.replace(/\D/g, "");
  for (let i = 0; i < 4; i++) {
    const idx = parseInt(seed.charAt(i) || String(i), 10) + i * 7;
    suffix += chars[idx % chars.length];
  }
  return `MP-${day}${month}${year}-${suffix}`;
}
...
export function ReceiptModal({ order, isOpen, onClose }: ReceiptModalProps) {
  ...
  const [printedReceipts, setPrintedReceipts] = useState<
    Set<"food" | "drinks" | "all">
  >(new Set());
  ...
  const triggerBrowserPrint = (type?: "food" | "drinks") => {
    if (!order) return;
    const { food: foodItems, drinks: drinkItems } = categorizeItems(
      order.items,
    );
    ...
    if (type === "food" || !type) {
      if (foodItems.length > 0)
        sections.push(buildPlainReceipt(foodItems, order, "food"));
    }
    if (type === "drinks" || !type) {
      if (drinkItems.length > 0)
        sections.push(buildPlainReceipt(drinkItems, order, "drinks"));
    }
    ...
    printDoc.write(`<html><head><title>Receipt</title><style>
      ...
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      pre { width:76mm; max-width:76mm; overflow:hidden; }
  ...
  useEffect(() => {
    ...
    const detectAndPrint = async () => {
      ...
      try {
        ...
        if (!allSuccess) {
          console.log(
            "Thermal print failed or no printer, using browser print fallback",
          );
