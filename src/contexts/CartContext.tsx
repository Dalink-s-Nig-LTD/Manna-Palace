...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // Synchronous lock: useRef so double-taps are blocked immediately (React state is async)
  ...
  const wasDisconnectedRef = useRef(false); // Track connection transitions for auto-sync
  const initialMountRef = useRef(true); // Prevent auto-sync on first mount
  ...
  const syncPendingOrders = useCallback(async () => {
    ...
    try {
      ...
      for (const qo of pending) {
        ...
        // Re-check this specific order's status (not all pending) to avoid race with direct submit
        try {
          const currentStatus = await orderQueue.getOrderStatus(qo.id);
          if (currentStatus === "synced" || currentStatus === "syncing") {
            console.log(
              `[CartContext] Order ${qo.id} already ${currentStatus}, skipping`,
            );
            continue;
          }
        } catch (e) {
        ...
        try {
          ...
          // Extract clientOrderId from the queued order data, with deterministic fallback
          const clientOrderId =
            (qo.order as Order & { clientOrderId?: string }).clientOrderId ||
            `fallback_${qo.id}_${qo.createdAt}`;

          console.log(
            `[CartContext] Syncing order ${qo.id} with clientOrderId: ${clientOrderId}`,
          );
  ...
  // Auto-sync ONLY when transitioning from disconnected → connected (not on initial mount)
  useEffect(() => {
    if (initialMountRef.current) {
      // Skip auto-sync on initial mount — just record the initial state
    ...
    // Only sync if we were previously disconnected (genuine reconnection)
    if (isConnected && wasDisconnectedRef.current && queueInitialized) {
      console.log(
        "[CartContext] Connection RESTORED from offline, auto-syncing...",
      );
  ...
  // Update pending count when queue is initialized, and heal any orders stuck in
  // "syncing" state from a previous session that crashed mid-flight.
  useEffect(() => {
    if (!queueInitialized) return;

    updatePendingCount();

    // Startup healer: reset stuck "syncing" orders to "pending" so the sync loop
    // retries them. createOrder on Convex is idempotent (clientOrderId dedup), so
    // re-sending an already-synced order simply returns { isDuplicate: true }.
    const healSyncingOrders = async () => {
      try {
        const stuck = await orderQueue.getSyncingOrders();
        if (stuck.length === 0) return;
        console.log(
          `[CartContext] Healing ${stuck.length} stuck "syncing" order(s) from previous session...`,
        );
        for (const qo of stuck) {
          await orderQueue.updateStatus(qo.id, "pending");
          console.log(`[CartContext] Reset ${qo.id} → "pending" for retry`);
        }
        await updatePendingCount();
      } catch (e) {
        console.error("[CartContext] Startup healer failed:", e);
      }
    };
  ...
  // Web localStorage cleanup on mount: remove orphaned pending orders that already exist in DB
  const findByClientOrderId = useQuery(
    api.orders.findByClientOrderId,
    // Only run this query if there are pending orders in localStorage (web only)
    (!isTauri &&
      ...
      (() => {
        try {
          ...
          const first = pending.find((o: any) => o.clientOrderId);
          return first ? { clientOrderId: first.clientOrderId } : "skip";
        } catch {
          return "skip";
        }
      })()) ||
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Remove orders older than 5 minutes that have no clientOrderId (can't verify)
      const fiveMinAgo = Date.now() - 5 * 60 * 1000;
      const cleaned = pending.filter((o: any) => {
        if (
          !o.clientOrderId &&
          o.timestamp &&
          new Date(o.timestamp).getTime() < fiveMinAgo
        ) {
          console.log(
            "[CartContext] Removing old orphaned localStorage order:",
            o.id,
          );
          return false;
        }
      ...
      if (cleaned.length !== pending.length) {
        if (cleaned.length > 0) {
          localStorage.setItem("pendingOrders", JSON.stringify(cleaned));
        } else {
      ...
      console.error(
        "[CartContext] Failed to clean localStorage pending orders:",
        e,
      );
    }
  }, [isTauri]);

  // If findByClientOrderId returned a match, clean that order from localStorage
  useEffect(() => {
    if (isTauri || !findByClientOrderId) return;
    if ("exists" in findByClientOrderId && findByClientOrderId.exists) {
      try {
        const pending = JSON.parse(
          localStorage.getItem("pendingOrders") || "[]",
        );
        const cleaned = pending.filter((o: any) => {
          if (o.clientOrderId && findByClientOrderId.exists) {
            // This specific clientOrderId exists in DB, remove it
            console.log(
              "[CartContext] Removing confirmed localStorage order with clientOrderId",
            );
            return false;
          }
          return true;
        ...
        console.error(
          "[CartContext] Failed to clean confirmed localStorage orders:",
          e,
        );
      }
    }
  }, [isTauri, findByClientOrderId]);
  ...
  const downloadDailySalesPDF = async (
    ...
    const lines: string[] = [];
    lines.push(center("Manna Palace"));
  ...
  const completeOrder = (
    ...
    // Synchronous ref check — blocks double-taps before React state can re-render
    if (processingOrderRef.current) {
      console.warn(
        "⚠️ Order already being processed (ref lock), ignoring duplicate request",
      );
    ...
    if (isTauri && queueInitialized) {
      (async () => {
        try {
          ...
          // Atomic: add with "syncing" status so auto-sync never picks it up
          const queueId = await orderQueue.addOrder(
            orderWithCashierCode,
            "syncing",
          );
          console.log(
            "[CartContext] Order saved to SQLite with status=syncing, queueId:",
            queueId,
          );

          // Also track in recentlySubmitted as a secondary guard (30s window)
          ...
          createOrder({
            ...
            .catch((error) => {
              console.error(
                "[CartContext] FAILED to save order to backend (reverting to pending for retry):",
                error,
              );
              clearTimeout(safetyTimeout);
              processingOrderRef.current = false;
              setIsProcessingOrder(false);
              // Revert to "pending" so sync loop can retry later
              orderQueue.updateStatus(queueId, "pending").catch(() => {});
            });
        } catch (queueError) {
          clearTimeout(safetyTimeout);
          processingOrderRef.current = false;
      ...
      // Web app: Save to localStorage first as backup with clientOrderId for dedup
      const orderWithCode = {
        ...order,
        cashierCode: cashierCode || "",
        clientOrderId,
      ...
      createOrder({
        ...
        .then((result) => {
          ...
          // Remove this order from pending by clientOrderId (more reliable than order.id)
          const pendingOrders = JSON.parse(
            localStorage.getItem("pendingOrders") || "[]",
          );
          const remainingOrders = pendingOrders.filter(
            (o: Order & { clientOrderId?: string }) =>
              o.clientOrderId !== clientOrderId && o.id !== order.id,
          );
