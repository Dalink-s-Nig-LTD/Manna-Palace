...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isTauri) {
      console.log("[CartContext] Not in Tauri, skipping orderQueue init");
    ...
    const initOrderQueue = async () => {
      try {
        console.log("[CartContext] Starting orderQueue initialization...");
        await orderQueue.init();
        console.log("[CartContext] ✅ OrderQueue initialized successfully");
        setQueueInitialized(true);
      } catch (error) {
        console.error(
          "[CartContext] ❌ Failed to initialize orderQueue:",
          error,
        );
        console.log(
          "[CartContext] Orders will use web-only (IndexedDB) storage",
        );
        // Don't block - let IndexedDB fallback work
        setQueueInitialized(false);
      }
    };
  ...
  const getTodaysOrders = async (): Promise<ReportOrder[]> => {
    ...
    // Both Tauri and Web: use Convex for primary order data
    // This ensures consistent reports across all platforms
    try {
      console.log("[CartContext] Fetching today's orders from Convex...");
      ...
      console.log(
        `[CartContext] ✅ getTodaysOrders: ${dbOrders.length} orders from Convex (ALL orders, will group by access code shift)`,
      );

      // Optional: Add unsynced local orders from queue if in Tauri
      if (isTauri && queueInitialized) {
        try {
          const queuedOrders = await orderQueue.getAllOrders();
            ...
            .filter(
              (qo) =>
                ...
                (qo.order as Order & { orderType?: string; cashierCode?: string })
                  .orderType !== "special" &&
                qo.status !== "synced", // Only unsynced orders
            )
            .map(
              (qo): ReportOrder => ({
                id: qo.id,
                ...
                paymentMethod: qo.order.paymentMethod,
                status: qo.order.status || "pending",
                cashierCode:
                  (qo.order as Order & { cashierCode?: string }).cashierCode ||
                  "LOCAL",
              }),
          ...
          if (localOrders.length > 0) {
            console.log(
              `[CartContext] Adding ${localOrders.length} unsynced local orders to report`,
            );
            dbOrders.push(...localOrders);
          }
        } catch (err) {
          console.warn("[CartContext] Could not fetch unsynced local orders:", err);
        }
      }
      ...
      console.error("[CartContext] Failed to fetch today's orders from Convex:", error);
      return [];
    }
  };
  ...
  const downloadDailySalesPDF = async (
    ...
    console.log(`[CartContext] Access code → Shift mapping:`, codeToShift);
    ...
    if (shiftFilter !== "all") {
      todaysOrders = todaysOrders.filter((order) => {
        ...
        console.log(
          `[CartContext] Order from ${order.cashierCode}: assigned to ${shift} shift`,
        );
    ...
    console.log(
      `[CartContext] Report filtered by ACCESS CODE shift "${shiftFilter}": ${todaysOrders.length} orders`,
    );
    ...
    // Set shift label based on filter (using access code assignments)
    let shift = "Full Day (All Access Codes)";
    if (shiftFilter === "morning") {
      shift = "Morning Shift (By Access Code)";
    } else if (shiftFilter === "afternoon") {
      shift = "Afternoon Shift (By Access Code)";
    } else if (shiftFilter === "evening") {
      shift = "Evening Shift (By Access Code)";
    }
