...
class IndexedDBQueue {
  ...
  async addOrder(order: Order, initialStatus: QueuedOrder["status"] = "pending"): Promise<string> {
...
class OrderQueue {
  ...
  async addOrder(order: Order, initialStatus: QueuedOrder["status"] = "pending"): Promise<string> {
    ...
    if (this.isTauri && sqliteDB) {
      try {
        const queueId = await sqliteDB.addOrder(order);
        // If initial status is not "pending", update immediately (atomic)
        if (initialStatus !== "pending") {
          await sqliteDB.updateStatus(queueId, initialStatus);
        }
        return queueId;
      } catch (error) {
        ...
        return this.indexedDBFallback.addOrder(order, initialStatus);
      }
    }
  ...
  async getOrderStatus(queueId: string): Promise<QueuedOrder["status"] | null> {
    await this.ensureInit();

    if (this.isTauri && sqliteDB) {
      try {
        const all = await sqliteDB.getAllOrders();
        const order = all.find((o) => o.id === queueId);
        return order?.status ?? null;
      } catch (error) {
        console.error("SQLite getOrderStatus failed:", error);
      }
    }

    const allOrders = await this.indexedDBFallback?.getAllOrders() || [];
    const order = allOrders.find((o) => o.id === queueId);
    return order?.status ?? null;
  }

  /** Return orders stuck in "syncing" (e.g. after an app crash). Only meaningful in Tauri/SQLite. */
  async getSyncingOrders(): Promise<QueuedOrder[]> {
    await this.ensureInit();
    if (this.isTauri && sqliteDB) {
      try {
        return await sqliteDB.getSyncingOrders();
      } catch (error) {
        console.error("SQLite getSyncingOrders failed:", error);
      }
    }
    // IndexedDB fallback: scan all orders for "syncing" status
    const all = await this.indexedDBFallback?.getAllOrders() || [];
    return all.filter(o => o.status === "syncing");
  }
}
