...
const DB_NAME = "mannapalaceCafeteriaDB";
const DB_VERSION = 1;
...
class OrderQueue {
  ...
  async init(): Promise<void> {
    if (this.initialized) {
      console.log("[OrderQueue] Already initialized");
      return;
    }

    // Check if running in Tauri
    this.isTauri = typeof window !== 'undefined' && '__TAURI__' in window;
    console.log("[OrderQueue] Init: isTauri =", this.isTauri);

    if (this.isTauri && sqliteDB) {
      try {
        console.log("[OrderQueue] Attempting SQLite initialization...");
        await sqliteDB.init();
        console.log("[OrderQueue] ✅ SQLite initialized successfully");
        this.initialized = true;
        return;
      } catch (error) {
        console.error("[OrderQueue] ❌ SQLite init failed:", error);
        console.warn("[OrderQueue] Will use IndexedDB fallback");
      }
    } else {
      console.log("[OrderQueue] Skipping SQLite - not in Tauri or sqliteDB not available");
    }

    // Fallback to IndexedDB
    this.indexedDBFallback = new IndexedDBQueue();
    await this.indexedDBFallback.init();
    console.log("[OrderQueue] ✅ IndexedDB fallback initialized");
    this.initialized = true;
  }
  ...
  async getAllOrders(): Promise<QueuedOrder[]> {
    ...
    if (this.isTauri && sqliteDB) {
      try {
        console.log("[OrderQueue] Attempting to fetch from SQLite...");
        const orders = await sqliteDB.getAllOrders();
        console.log("[OrderQueue] ✅ SQLite getAllOrders returned:", orders.length, "orders");
        return orders;
      } catch (error) {
        console.error("[OrderQueue] ❌ SQLite getAllOrders failed:", error);
        console.warn("[OrderQueue] Falling back to IndexedDB...");
      }
    } else {
      console.log("[OrderQueue] Not using SQLite - isTauri:", this.isTauri, "sqliteDB:", !!sqliteDB);
    }
