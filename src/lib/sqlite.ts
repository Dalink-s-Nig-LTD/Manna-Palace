...
const loadDatabase = async (): Promise<TauriSQLDatabase> => {
  ...
  if (databaseLoadError) throw databaseLoadError;

  try {
    console.log("[SQLite] Attempting to load @tauri-apps/plugin-sql...");
    const mod = await import("@tauri-apps/plugin-sql");
    if (!mod.default) {
      throw new Error("Module loaded but no default export found");
    }
    Database = mod.default;
    console.log("[SQLite] ✅ Successfully loaded @tauri-apps/plugin-sql");
    return Database;
  } catch (error) {
    databaseLoadError = error instanceof Error ? error : new Error(String(error));
    console.error("[SQLite] ❌ Failed to load @tauri-apps/plugin-sql:", error);
...
class SQLiteOrderDB {
  ...
  async getAllOrders(): Promise<QueuedOrder[]> {
    ...
    if (rows.length > 0) {
      console.log("[SQLiteDB] Sample order:", {
        id: rows[0].id,
        status: rows[0].status,
        created_at: rows[0].created_at,
        order_sample: JSON.parse(rows[0].order_data).cashierCode,
      });
    }
    return rows.map(this.rowToQueuedOrder);
  }
  ...
  // ---- Balance and Deduction methods ----

  async updateCustomerBalance(customerId: string, newBalance: number): Promise<void> {
    const db = await this.ensureDB();
    await db.execute(
      `UPDATE customers_cache SET balance = $1, updated_at = $2 WHERE customer_id = $3`,
      [newBalance, Date.now(), customerId]
    );
    console.log(`[SQLiteDB] Customer ${customerId} balance updated to ₦${newBalance}`);
  }

  async addDeductionOrder(order: {
    _id: string;
    customerId: string;
    amount: number;
    reason?: string;
    ...
    console.log(`[SQLiteDB] Deduction order ${order._id} created for ₦${order.amount}`);
  }
...
export const sqliteDB: SQLiteOrderDB | null = isTauriEnvironment() ? (() => {
  ...
  console.log("[sqliteDB export] Initialized:", db ? "✅" : "❌");
  return db;
})() : (() => {
  console.log("[sqliteDB export] Skipping - not in Tauri");
  return null;
})();

console.log(`[SQLiteDB Module] Loaded. Tauri: ${isTauriEnvironment()}, sqliteDB instance: ${sqliteDB ? "✅" : "❌"}`);
