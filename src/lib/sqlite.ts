...
class SQLiteOrderDB {
  ...
  async init(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        // Migrate: add client_order_id column if missing (SQLite ALTER TABLE doesn't support IF NOT EXISTS)
        try {
          await this.db.execute(`ALTER TABLE order_queue ADD COLUMN client_order_id TEXT`);
          console.log('[SQLiteDB] Added client_order_id column to order_queue');
        } catch (_e) {
          // Column already exists — this is expected on all but the first run after migration
        }
        // Unique index allows one row per clientOrderId, NULLs are not considered equal so old rows are safe
        await this.db.execute(
          `CREATE UNIQUE INDEX IF NOT EXISTS idx_oq_client_order_id ON order_queue(client_order_id) WHERE client_order_id IS NOT NULL`
        );
        console.log('[SQLiteDB] client_order_id unique index ensured');
  ...
  async addOrder(order: Order): Promise<string> {
    ...
    // Extract clientOrderId stored on the order object by CartContext
    const clientOrderId: string | null = (order as any).clientOrderId || null;

    // Dedup: if we already have this clientOrderId, return the existing queueId
    if (clientOrderId) {
      const existing = await this.getByClientOrderId(clientOrderId);
      if (existing) {
        console.log(`[SQLiteDB] Duplicate blocked — clientOrderId already queued: ${clientOrderId} (queueId: ${existing.id})`);
        return existing.id;
      }
    }
    ...
    console.log(`[SQLiteDB] Adding order ${queueId} to queue (clientOrderId: ${clientOrderId}, createdAt: ${createdAt})`);
    await db.execute(
      `INSERT INTO order_queue (id, order_data, status, attempts, created_at, client_order_id) VALUES ($1, $2, $3, $4, $5, $6)`,
      [queueId, JSON.stringify(order), "pending", 0, createdAt, clientOrderId]
    );
  ...
  /** Return all orders currently stuck in "syncing" state (e.g. after an app crash). */
  async getSyncingOrders(): Promise<QueuedOrder[]> {
    const db = await this.ensureDB();
    const rows: OrderQueueRow[] = await db.select(
      `SELECT * FROM order_queue WHERE status = 'syncing' ORDER BY created_at ASC`
    );
    console.log(`[SQLiteDB] Found ${rows.length} stuck 'syncing' orders`);
    return rows.map(this.rowToQueuedOrder);
  }

  /** Look up a queued order by its clientOrderId (stored in the dedicated column). */
  async getByClientOrderId(clientOrderId: string): Promise<QueuedOrder | null> {
    const db = await this.ensureDB();
    const rows: OrderQueueRow[] = await db.select(
      `SELECT * FROM order_queue WHERE client_order_id = $1 LIMIT 1`,
      [clientOrderId]
    );
