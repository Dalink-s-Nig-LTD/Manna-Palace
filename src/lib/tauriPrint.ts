...
function generateReceiptText(order: Order, type?: 'food' | 'drinks'): string {
  ...
  items.forEach(item => {
    ...
    const price = `N${itemTotal}`;
    const spaces = Math.max(1, 48 - itemLine.length - price.length);
    lines.push(itemLine + ' '.repeat(spaces) + price);
  });
  
  lines.push('------------------------------------------------');
  const totalStr = `N${total.toLocaleString()}`;
  lines.push('TOTAL' + ' '.repeat(48 - 5 - totalStr.length) + totalStr);
  lines.push('================================================');
