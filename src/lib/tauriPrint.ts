...
import {
  ...
  buildReportCommands,
  buildSpecialOrderReceiptCommands,
  type ReceiptData,
  type ReportData,
  type SpecialOrderReceiptData,
} from "./escpos";
 ...
 */
function categorizeItems(items: CartItem[]): {
  food: CartItem[];
  drinks: CartItem[];
} {
  const food: CartItem[] = [];
...
function generateReceiptText(order: Order, type?: "food" | "drinks"): string {
  ...
  const items =
    type === "food" ? food : type === "drinks" ? drinks : order.items;
 ...
 */
export async function printReceiptTauri(
  order: Order,
  type?: "food" | "drinks",
): Promise<boolean> {
  const invoke = await getTauriInvoke();
  ...
  try {
    ...
    console.warn(
      "Thermal printer failed, returning false for styled browser fallback:",
      error,
    );
    return false;
  }
 ...
 */
export async function printSpecialOrderReceiptTauri(
  data: SpecialOrderReceiptData,
): Promise<boolean> {
  const invoke = await getTauriInvoke();
  if (!invoke) {
    console.warn("Tauri not available, falling back to browser print");
  ...
  try {
    await invoke("print_escpos_cmd", { data: commands });
    console.log(
      "Special order receipt printed successfully via thermal printer",
    );
    return true;
  } catch (error) {
