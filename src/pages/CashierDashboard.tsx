  ...
  return (
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      ...
      <main className="flex-1 flex flex-row overflow-hidden">
        ...
        {!isMobile && (
          <div className="w-80 lg:w-96 border-l border-border hidden md:flex flex-col overflow-hidden bg-card/50 p-4 lg:p-5">
            <Cart />
          </div>
        )}
      </main>
              ...
              {hasLocalData &&
                (unsyncedCount > 0 ? (
                  <div className="flex items-center gap-2 p-3 rounded-lg border border-orange-200 bg-orange-50 text-orange-700 text-sm">
                    ...
                    <span>
                      Includes <strong>{unsyncedCount}</strong> unsynced local
                      order{unsyncedCount !== 1 ? "s" : ""} from SQLite
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 p-3 rounded-lg border border-green-200 bg-green-50 text-green-700 text-sm">
                    ...
                    <span>
                      All <strong>{localOrderCount}</strong> local order
                      {localOrderCount !== 1 ? "s" : ""} synced successfully
                    </span>
                  </div>
                ))}
                  ...
                  <div className="p-4 border rounded-lg bg-muted/50">
                    ...
                    {Object.keys(shiftSales.unassigned.byAccessCode).length >
                      ...
                      <div className="mt-3 pt-3 border-t border-border">
                        ...
                        <div className="space-y-1">
                          {Object.entries(
                            shiftSales.unassigned.byAccessCode,
                          ).map(([code, data]) => (
                            <div
                              key={code}
                              className="flex justify-between text-sm"
